        ...
        <div className="h-screen w-screen bg-[rgba(234,235,237,1)] dark:bg-[rgba(23,23,56,1)] overflow-x-hidden">
            <div className="h-full min-md:w-2/3 max-md:w-full flex items-center justify-center">
                <div className="w-3/5 min-w-150 max-w-200 h-120 flex flex-col items-center text-black border-r-1 border-r-black dark:border-r-white max-md:border-none">
                    <form action={signup} method="POST" className="w-1/2 h-5/6 m-auto flex flex-col justify-center gap-y-2" autoComplete="off">
                        <Image src="/logo/logo1.png" alt="Logo" width={500} height={500} className="mt-13 flex justify-center dark:invert"/>
                        <input type="text" name="username" className="w-full h-1/6 p-1 mt-2 bg-white text-lg rounded-lg border-black border-1" placeholder="Username" minLength={5} maxLength={32} required></input>
                        <input type="email" name="email" className="w-full h-1/6 p-1 bg-white text-lg rounded-lg border-black border-1" placeholder="Email" minLength={6} maxLength={255} required></input>
                        <input type="password" name="password" className="w-full h-1/6 p-1 bg-white text-lg rounded-lg border-black border-1" placeholder="Password" minLength={10} required></input>
                        <input type="password" name="confirm" className="w-full h-1/6 p-1 bg-white text-lg rounded-lg border-black border-1" placeholder="Confirm Password" minLength={10} required></input>
